...
public class Contact {
    ...
    public static String formatNameAndNumber(String name, String number, String numberE164) {
        ...
        if (!SmsHelper.isEmailAddress(number)) {
            formattedNumber = PhoneNumberUtils.formatNumber(number, numberE164, AppBase.getApplication().getCurrentCountryIso());
        }
    ...
    public synchronized void setNumber(String number) {
        if (!SmsHelper.isEmailAddress(number)) {
            mNumber = PhoneNumberUtils.formatNumber(number, mNumberE164, AppBase.getApplication().getCurrentCountryIso());
        } else {
            mNumber = number;
        }
    ...
    private static class ContactsCache {
        ...
        private boolean contactChanged(Contact orig, Contact newContactData) {
            ...
            if (!oldName.equals(newName)) {
                if (Log.isLoggable(LogTag.CONTACT, Log.DEBUG)) {
                    MLog.d(TAG, String.format("name changed: %s -> %s", oldName, newName));
                }
                return true;
            }
            ...
            if (!oldLabel.equals(newLabel)) {
                if (Log.isLoggable(LogTag.CONTACT, Log.DEBUG)) {
                    MLog.d(TAG, String.format("label changed: %s -> %s", oldLabel, newLabel));
                }
                return true;
            }
        ...
        private Contact getContactInfoForPhoneNumber(String number) {
            ...
            if (!TextUtils.isEmpty(normalizedNumber) && !TextUtils.isEmpty(minMatch)) {
                ...
                String numberE164 = PhoneNumberUtils.formatNumberToE164(
                        number, AppBase.getApplication().getCurrentCountryIso());
                String selection;
                String[] args;
