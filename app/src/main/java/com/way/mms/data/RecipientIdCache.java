...
public class RecipientIdCache {
    ...
    public static void fill() {
        ...
        if (c == null) {
            MLog.w(TAG, "null Cursor in fill()");
            return;
        }
    ...
    public static List<Entry> getAddresses(String spaceSepIds) {
        synchronized (sInstance) {
            ...
            for (String id : ids) {
                ...
                if (number == null) {
                    MLog.w(TAG, "RecipientId " + longId + " not in cache!");
                ...
                if (TextUtils.isEmpty(number)) {
                    MLog.w(TAG, "RecipientId " + longId + " has empty number!");
                } else {
        ...
        for (Contact contact : contacts) {
            ...
            synchronized (sInstance) {
                ...
                if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
                    MLog.d(TAG, "[RecipientIdCache] updateNumbers: contact=" + contact +
                            ", wasModified=true, recipientId=" + recipientId);
                    MLog.d(TAG, "   contact.getNumber=" + number1 +
    ...
    private void updateCanonicalAddressInDb(long id, String number) {
        if (LogTag.VERBOSE || Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
            MLog.d(TAG, "[RecipientIdCache] updateCanonicalAddressInDb: id=" + id +
                    ", number=" + number);
        }
    ...
    public static void dump() {
        ...
        synchronized (sInstance) {
            MLog.d(TAG, "*** Recipient ID cache dump ***");
            for (Long id : sInstance.mCache.keySet()) {
                MLog.d(TAG, id + ": " + sInstance.mCache.get(id));
            }
        }
    }

    public static void canonicalTableDump() {
        MLog.d(TAG, "**** Dump of canoncial_addresses table ****");
        Context context = sInstance.mContext;
        Cursor c = SqliteWrapper.query(context, context.getContentResolver(),
                sAllCanonical, null, null, null, null);
        if (c == null) {
            MLog.w(TAG, "null Cursor in content://mms-sms/canonical-addresses");
        }
        try {
            while (c.moveToNext()) {
                // TODO: don't hardcode the column indices
                long id = c.getLong(0);
                String number = c.getString(1);
                MLog.d(TAG, "id: " + id + " number: " + number);
            }
        } finally {
            c.close();
        }
    ...
    public static void insertCanonicalAddressInDb(final Context context, String number) {
        if (LogTag.VERBOSE || Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
            MLog.d(TAG, "[RecipientIdCache] insertCanonicalAddressInDb: number=" + number);
        }
