...
import com.way.mms.interfaces.ILiveView;
 ...
 * LiveViewManager.registerView(WayPreference.THEME, key -> {
 * // Change button color
 * }
 * <p>
...
public abstract class LiveViewManager {
     ...
     */
    private static final HashMap<String, WeakHashMap<Object, Set<ILiveView>>> sViews = new HashMap<>();
    ...
    private static final HashSet<String> sExcludedPrefs = new HashSet<>(Arrays.asList(
            WayPreference.THEME.getKey(),
            WayPreference.BACKGROUND.getKey()
    ));
    ...
    /**
     * Register a view to be updated when a WayPreference is changed
     ...
     */
    public static void registerView(WayPreference preference, Object parent, ILiveView view) {
        synchronized (sViews) {
            if (sViews.containsKey(preference.getKey())) {
                WeakHashMap<Object, Set<ILiveView>> parents = sViews.get(preference.getKey());
                ...
                WeakHashMap<Object, Set<ILiveView>> set = new WeakHashMap<>();
     ...
     */
    public static void refreshViews(WayPreference preference) {
    ...
    private static void refreshViews(String key) {
        synchronized (sViews) {
            if (sViews.get(key) != null) {
                for (Set<ILiveView> views : sViews.get(key).values()) {
                    for (ILiveView view : views) {
                        view.refresh(key);
                    }
                }
            }
